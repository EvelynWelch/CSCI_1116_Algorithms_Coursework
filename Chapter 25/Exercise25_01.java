...
public class Exercise25_01 {
	...
	public Exercise25_01() {
		...
		BinaryTree<String> tree1 = new BinaryTree<String>(new String[] { "Tom", "George", "Jean", "Jane", "Kevin",
...
//		BinaryTree<Integer> tree2 = new BinaryTree<Integer>(new Integer[] { 60, 55, 100, 45, 57, 59, 100, 67, 107, 101 });
	...
	public class BinaryTree<E extends Comparable<E>> extends AbstractTree<E> {
		...
		/**
		 * Return the depth of this binary tree. Depth is the number of the nodes in the
		...
		private int height(TreeNode<E> node) {
			// if no root node is found the depth is -1
			if (node == null) {
				return -1;
			} else {
				// recurse through the left and right tree
		...
		/**
		 * Displays the nodes in breadth-first traversal Breadth-first traversal
		 * explores the tree level-by-level Starting at the root, explore each child
		 * left to right then their children left to right and so on
		 */
		public void breadthFirstTraversal() {
			...
			// While there is a current or there are nodes in the queue
			while (queue.size() != 0 || current != null) {
				// if there is a current print it
				if (current != null) {
					System.out.print(current.element + " ");
				}
				// if there is a left tree add it to queue
				...
				// get the queue[0] and set it to current.
				current = queue.poll();
			}
		}
