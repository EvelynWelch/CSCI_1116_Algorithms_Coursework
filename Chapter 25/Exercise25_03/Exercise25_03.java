...
//		int[] list = { 10, 8, 6, 4, 2, 1, 3, 5, 7, 9, 11, 15, 122, 1234, 32, 123, 18, 21 };
//		for (int i : list) {
//			tree.insert(i);
//		}
//		tree.inorder();
...
//
	...
	public static class BST<E extends Comparable<E>> extends AbstractTree<E> {
		...
		public void nonRecursiveInorder() {
			...
			if (root == null)
				return;
			// create a stack and get the root node
			Stack<TreeNode<E>> inOrder = new Stack<>();
			TreeNode<E> current = getRoot();
			// while there is a current node, or if there are nodes in the stack
			while (current != null || inOrder.size() > 0) {
				// get the bottom of the left tree
				while (current != null) {
					inOrder.push(current);
					current = current.left;
				}
				// current is null here so we grab the last element off the stack
				current = inOrder.pop();
				System.out.print(current.element + " ");
				// we have hit the end of the left tree so we move on to the right
				current = current.right;
			}
		}
	}
